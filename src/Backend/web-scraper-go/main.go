package main

import (
	// "fmt"
	"encoding/json"
	"fmt"
	"net/http"

	// b "lala/bfs"
	"time"
	b "web-scraper/bfs"
)

type RequestInfo struct {
	Algorithm  string `json:"algorithm"`
	StartPage  string `json:"startPage"`
	TargetPage string `json:"targetPage"`
}

type ResponseInfo struct {
	Status        string      `json:"status"`
	Error_Message string      `json:"err"`
	Graph         b.GraphView `json:"graph"`
	ResultDepth   int         `json:"depth"`
	ExecutionTime float64     `json:"time"`
}

// Untuk menerima request dari frontend
var reqInfo RequestInfo

// Untuk memberi response ke frontend
var respInfo ResponseInfo

var wiki string = "/wiki/"

func main() {
	http.HandleFunc("/api/process", request_response_Handler)

	// var start_title = "Neuroscience"
	// var start_title = "Coffee"
	// var start_title = "Joko_Widodo"
	// var start_title = "Indonesia"
	// var page string = wiki + start_title
	// var start_page = []string{page}
	// startTime := time.Now()
	// depth 1 - Neuroscience, Jokowi
	// b.BFS(start_page, "Anatomy")
	// depth 2 - Neuroscience
	// b.BFS(start_page, "Antenna_(biology)")
	// depth 2 - Jokowi
	// b.BFS(start_page, "Adolf_Hitler")
	// depth 3 - Neuroscience
	// b.BFS(start_page, "Springtail")
	// Failed(?)
	// b.BFS(start_page, "Collophore")
	// b.BFS(start_page, "Category:Non-alcoholic_drinks")
	// b.BFS(start_page, "Special:BookSources/978-0521570480")
	// endTime := time.Now()
	// fmt.Println("Executed in ", float64(endTime.Sub(startTime).Seconds()*1000), " ms")
	// fmt.Printf("%v\n", b.GraphSolusi)
	fmt.Println("Server listening on port 8000")
	http.ListenAndServe(":8000", nil)
}

type Tes struct {
	Text   string `json:"text"`
	Number int    `json:"number"`
}

func request_response_Handler(w http.ResponseWriter, r *http.Request) {
	// REQUEST INPUT FROM FRONTEND
	// fmt.Println("Request Method:", r.Method)
	// fmt.Println("Request Path:", r.URL.Path)
	// fmt.Println("Request Referrer:", r.Referer())
	// fmt.Println("Request Header:", r.Header)
	if r.Method == "POST" {
		// err := json.NewDecoder(r.Body).Decode(&input)
		err := json.NewDecoder(r.Body).Decode(&reqInfo)
		if err != nil {
			http.Error(w, "Error decoding input.", http.StatusBadRequest)
		} else {
			fmt.Println("Received input:")
			fmt.Println("Algorithm: ", reqInfo.Algorithm)
			fmt.Println("Start: ", reqInfo.StartPage)
			fmt.Println("Target: ", reqInfo.TargetPage)
			fmt.Println("Ready to GET")
			// ALGORITMA UTAMA
			if reqInfo.Algorithm == "bfs" {
				// JSON testing
				// {
				// 	"algorithm": "bfs",
				// 	"startPage": "Neuroscience",
				// 	"targetPage": "Antenna Biology"
				// }

				// ALGORITMA BFS
				startTime := time.Now()
				fmt.Println("Processing BFS...")
				b.BFS([]string{wiki + reqInfo.StartPage}, reqInfo.TargetPage)
				endTime := time.Now()
				respInfo.Status = b.Status
				respInfo.Error_Message = b.Err_msg
				respInfo.Graph = b.GraphSolusi
				respInfo.ResultDepth = b.ResultDepth
				respInfo.ExecutionTime = float64(endTime.Sub(startTime).Seconds() * 1000000 / 1000)
				// RESPONSE TO FRONTEND
			} else if reqInfo.Algorithm == "ids" {
				// ALGORITMA IDS
				fmt.Println("IDS HERE")
			}
			w.Header().Set("Content-Type", "application/json")
			w.Header().Set("Access-Control-Allow-Origin", "*")
			w.Header().Set("Access-Control-Allow-Methods", "POST, GET, OPTIONS")
			// w.Header().Set("Access-Control-Allow-Headers", "Content-Type")
			err := json.NewEncoder(w).Encode(respInfo)
			b.ResetData()
			fmt.Println("Reset Data")
			if err != nil {
				http.Error(w, "Error encoding data", http.StatusInternalServerError)
			}
		}
	} else if r.Method == "OPTIONS" {
		w.Header().Set("Access-Control-Allow-Origin", "http://localhost:3000")
		w.Header().Set("Access-Control-Allow-Methods", "POST, GET, OPTIONS")
		w.Header().Set("Access-Control-Allow-Headers", "Content-Type")
		w.WriteHeader(http.StatusOK)
	} else {
		fmt.Println("NOT POST", r.Method)
		// ALGORITMA UTAMA
		if reqInfo.Algorithm == "bfs" {
			// JSON testing
			// {
			// 	"algorithm": "bfs",
			// 	"startPage": "Neuroscience",
			// 	"targetPage": "Antenna Biology"
			// }

			// ALGORITMA BFS
			startTime := time.Now()
			fmt.Println("Processing BFS...")
			b.BFS([]string{wiki + reqInfo.StartPage}, reqInfo.TargetPage)
			endTime := time.Now()
			respInfo.Status = b.Status
			respInfo.Error_Message = b.Err_msg
			respInfo.Graph = b.GraphSolusi
			respInfo.ResultDepth = b.ResultDepth
			respInfo.ExecutionTime = float64(endTime.Sub(startTime).Seconds() * 1000000 / 1000)
			// RESPONSE TO FRONTEND
		} else if reqInfo.Algorithm == "ids" {
			// ALGORITMA IDS
			fmt.Println("IDS HERE")
		}
		w.Header().Set("Content-Type", "application/json")
		w.Header().Set("Access-Control-Allow-Origin", "*")
		w.Header().Set("Access-Control-Allow-Methods", "POST, GET, OPTIONS")
		// w.Header().Set("Access-Control-Allow-Headers", "Content-Type")
		err := json.NewEncoder(w).Encode(respInfo)
		b.ResetData()
		fmt.Println("Reset Data")
		if err != nil {
			http.Error(w, "Error encoding data", http.StatusInternalServerError)
		}
	}

	// w.WriteHeader(http.StatusOK)
	// w.Header().Set("Content-Type", "application/json")
	// w.Header().Set("Access-Control-Allow-Origin", "http://localhost:3000")
	// w.Header().Set("Access-Control-Allow-Methods", "POST, GET, OPTIONS")
	// w.Header().Set("Access-Control-Allow-Headers", "Content-Type")
	// err := json.NewEncoder(w).Encode(respInfo)
	// if err != nil {
	// 	http.Error(w, "Error encoding data", http.StatusInternalServerError)
	// } else {
	// 	fmt.Println("Ready to GET")
	// }

	// RESPONSE TO FRONTEND
	// jsonBytes, err := json.Marshal(b.GraphSolusi)
	// if err != nil {
	// 	// Handle error
	// 	http.Error(w, err.Error(), http.StatusInternalServerError)
	// 	return
	// }
	// w.Header().Set("Content-Type", "application/json")
	// w.Header().Set("Access-Control-Allow-Origin", "*") // Allow requests from any origin (replace with your frontend origin if needed)
	// w.Write(jsonBytes)

	// jsonBytes, _ := json.Marshal(b.GraphSolusi)
	// w.WriteHeader(http.StatusOK)
	// w.Header().Set("Content-Type", "application/json")
	// w.Header().Set("Access-Control-Allow-Origin", "http://localhost:3000")
	// w.Header().Set("Access-Control-Allow-Methods", "POST, GET, OPTIONS")
	// w.Header().Set("Access-Control-Allow-Headers", "Content-Type")
	// err1 := json.NewEncoder(w).Encode(b.GraphSolusi)
	// w.Write(jsonBytes)

	// if err1 != nil {
	// 	http.Error(w, "Error encoding data", http.StatusInternalServerError)
	// 	return
	// }
	// }
}

// func getMessage(w http.ResponseWriter, r *http.Request) {
// 	message := Message{Text: "Hello from Go backend!"}
// 	w.Header().Set("Content-Type", "application/json")
// 	w.Header().Set("Access-Control-Allow-Origin", "*") // Allow requests from any origin (replace with your frontend origin if needed)
// 	json.NewEncoder(w).Encode(message)
// }
